    ...
    const overlayMenuHTML = menuItems.map(id => {
        ...
        if (id === 'sip-panel') {
            ...
            <div class="mb-4">
               ...
               <div id="accordion-${id}" class="accordion-content ${isOpen ? 'open' : ''}">
                   <div class="pl-4 border-l-2 border-brand-orange ml-1 flex flex-col">
                       <a href="#${id}" class="text-base sm:text-lg text-gray-400 hover:text-brand-orange cursor-pointer font-medium py-2 select-none" onclick="window.filterAndNavigate('${id}', 'all', event)">${t().allProjectsTitle}</a>
                       ${categories.map(cat => `<a href="#${id}" class="text-base sm:text-lg text-gray-400 hover:text-brand-orange cursor-pointer font-medium py-2 select-none" onclick="window.filterAndNavigate('${id}', '${cat.id}', event)">${cat[state.lang]}</a>`).join('')}
                   </div>
               </div>
            </div>`;
        } else {
            return `
            <div class="mb-4">
                <div class="w-[220px] sm:w-[280px] flex items-center justify-between select-none">
                    <a href="#${id}" onclick="navigate('${id}', event)" class="text-2xl sm:text-3xl font-semibold text-gray-300 hover:text-white transition block w-full pointer-events-auto">${label}</a>
                </div>
            </div>`;
        }
    }).join('');
    ...
    DOM.header.innerHTML = `
        <div class="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between relative z-50 pointer-events-auto">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 md:space-x-4 ml-auto shrink-0">
                ...
                <button onclick="window.toggleMobileMenu()" aria-label="Menü" class="w-10 h-10 md:w-12 md:h-12 bg-[#1a1a1a] rounded-full flex items-center justify-center text-white hover:bg-brand-orange shadow-lg transition-all duration-300 btn-press focus:outline-none shrink-0 z-[101] select-none">
                    <i class="fas fa-bars text-base sm:text-lg pointer-events-none"></i>
                </button>
            </div>
        </div>

        <div id="vg-overlay-bg" onclick="window.closeMenuFromOutside(event)" class="vg-overlay pointer-events-auto">
            <div class="w-full p-6 sm:p-8 flex justify-end items-center shrink-0">
                <div class="flex items-center space-x-4 md:space-x-6">
                    ...
                    <button onclick="window.toggleMobileMenu()" aria-label="Kapat" class="w-10 h-10 sm:w-12 sm:h-12 bg-white/10 rounded-full flex items-center justify-center text-white hover:bg-red-500 transition-all duration-300 btn-press z-[105] select-none">
            ...
            <div class="w-full flex-grow px-8 sm:px-16 md:px-24 lg:px-40 flex flex-col justify-start pt-6 sm:pt-10 pb-24 overflow-y-auto no-scrollbar">
                ${overlayMenuHTML}
                <div class="mt-8 pt-8 border-t border-gray-800 flex items-center space-x-6 w-[220px] sm:w-[280px] select-none">
                    <div class="flex w-full justify-between items-center">
                        <span class="text-xl text-gray-400 font-semibold pointer-events-none">Dil</span>
                        <div class="flex space-x-3 bg-white/5 rounded-full px-4 py-2">
                            ...
                            <span class="text-gray-700 pointer-events-none">|</span>
...
function renderHomePage() {
    ...
    const projectsHTML = pageProjects.map((project, index) => `
        <a href="#${project.id}" class="project-card bg-white border border-gray-100 cursor-pointer rounded-2xl btn-press overflow-hidden flex flex-col group block shadow-sm reveal-element delay-${(index%3+1)*100}" onclick="navigate('${project.id}', event)">
    ...
    const processHTML = (t().processSteps || []).map((step, idx) => `
        <div class="flex flex-col items-center text-center relative z-10 group reveal-element delay-${(idx%4+1)*100}">
    ...
    const faqHTML = (t().faq || []).map((f, idx) => `
        <details class="group bg-white rounded-2xl shadow-sm border border-gray-100 mb-4 overflow-hidden h-max break-inside-avoid inline-block w-full reveal-element">
    ...
    const categoriesHTML = categoryKeys.map((catKey, index) => {
        ...
        const bgImage = firstProject ? firstProject.mainImage : siteConfig.homeHero.backgroundImage;
    ...
    DOM.content.innerHTML = `
        ...
        <div id="categories-section" class="bg-white relative z-30 w-full py-10 sm:py-16 border-b border-gray-100">
            <div class="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-8 sm:mb-10 reveal-element">
        ...
        <div class="bg-gray-50 relative z-20 w-full py-10 sm:py-16 border-b border-gray-200">
            <div class="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-10 reveal-element">
        ...
        <div class="bg-white relative z-20 w-full py-10 sm:py-16 border-b border-gray-200">
            <div class="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
                    <div class="w-full lg:w-1/2 lg:sticky lg:top-24 reveal-element">
                    ...
                    <div class="w-full lg:w-1/2">
                        <div class="space-y-8 sm:space-y-12">
                            <div class="flex flex-col sm:flex-row items-start sm:gap-5 group reveal-element delay-100">
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:gap-5 group reveal-element delay-200">
                                <div class="w-14 h-14 rounded-2xl bg-orange-50 flex items-center justify-center shrink-0 mb-3 sm:mb-0 group-hover:bg-brand-orange transition-colors">
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:gap-5 group reveal-element delay-300">
                                <div class="w-14 h-14 rounded-2xl bg-orange-50 flex items-center justify-center shrink-0 mb-3 sm:mb-0 group-hover:bg-brand-orange transition-colors">
        ...
        <div id="featured-projects" class="bg-gray-50 relative z-20 w-full py-10 sm:py-16">
            <div class="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-8 sm:mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 reveal-element">
                    <div class="text-left">
        ...
        <div class="bg-[#1a201c] relative z-20 w-full py-12 sm:py-16 overflow-hidden border-t-4 border-brand-orange reveal-element">
...
function renderSipPanelPage() {
    ...
    const advantagesHTML = data.advantages.map((adv, idx) => `
        <div class="flex flex-col gap-3 group reveal-element delay-${(idx%2)*100}">
    ...
    const specsHTML = data.technicalSpecs.map((spec, idx) => `
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 border-b border-gray-100 last:border-0 gap-2 sm:gap-6 reveal-element">
    ...
    const usageHTML = data.usageAreas ? data.usageAreas.map((item, idx) => {
        ...
        return `
        <div class="flex items-start gap-3 bg-gray-50 p-5 rounded-2xl border border-gray-100 reveal-element delay-${(idx%2)*100}">
    ...
    DOM.content.innerHTML = `
        <div class="relative w-full h-[60vh] md:h-[70vh] flex overflow-hidden bg-black">
            ...
            <div class="absolute inset-0 flex flex-col justify-center items-center text-center px-4 sm:px-6 z-10 pt-16 reveal-element">
        ...
        <div class="bg-white py-12 sm:py-16 px-4 sm:px-6 relative z-30">
            <div class="max-w-[1000px] mx-auto space-y-12 sm:space-y-20">
                ...
                <div class="w-full h-px bg-gray-100 reveal-element"></div>
                ...
                ${data.futureTitle ? `
                <div class="bg-[#1a201c] p-6 sm:p-10 md:p-12 rounded-[2rem] shadow-2xl text-center relative overflow-hidden reveal-element">
                ...
                <div class="bg-gray-50 rounded-[2rem] p-6 sm:p-10 md:p-12 border border-gray-200 text-center relative overflow-hidden mt-10 sm:mt-16 reveal-element">
...
function renderAboutPage() {
    ...
    const featuresHTML = data.features.map((f, idx) => `
        <div class="flex flex-col reveal-element delay-${(idx%3)*100}">
    ...
    const paragraphsHTML = data.paragraphs.map(p => `
        <p class="text-sm sm:text-base lg:text-lg text-gray-600 leading-relaxed font-medium mb-4 last:mb-0 reveal-element">${p}</p>
    `).join('');
    ...
    DOM.content.innerHTML = `
        <div class="bg-white min-h-screen pt-28 sm:pt-36 pb-16">
            <div class="max-w-[1000px] w-full mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="w-full h-px bg-gray-100 mb-12 reveal-element"></div>
                ...
                <div class="mt-16 bg-gray-50 rounded-[2rem] p-6 sm:p-10 border border-gray-200 text-center reveal-element">
    ...
    DOM.content.innerHTML = `
        <div class="bg-gray-50 min-h-screen pt-28 pb-12 sm:pb-16 px-4 sm:px-6">
            <div class="max-w-5xl mx-auto">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 bg-white p-6 sm:p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100 reveal-element">
...
function renderProjectsPage(pageId) {
    ...
    const projectsHTML = pageProjects.length > 0 ? pageProjects.map((project, idx) => `
        <a href="#${project.id}" class="group relative aspect-[4/3] rounded-3xl overflow-hidden cursor-pointer btn-press block shadow-md hover:shadow-xl transition-all duration-400 reveal-element delay-${(idx%3)*100}" onclick="window.navigate('${project.id}', event)">
...
function renderProjectDetail(projectId) {
    ...
    if(relatedProjects.length > 0) {
        relatedHTML = `
            <div class="mt-12 sm:mt-16 reveal-element">
        ...
        <div class="max-w-[1300px] w-full mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-36 pb-12 relative z-10">
            ...
            <div class="mb-5 sm:mb-6 flex flex-wrap items-center justify-between gap-3 reveal-element">
                <div class="flex items-center text-xs sm:text-sm font-semibold text-gray-500 space-x-2">
                    ...
                    <a href="#${project.pageMenu}" onclick="window.navigate('${project.pageMenu}', event)" class="hover:text-brand-orange transition-colors">${parentMenuLabel}</a>
                    <i class="fas fa-chevron-right text-[10px] text-gray-300"></i>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-start">
                ...
                <div class="lg:col-span-7 flex flex-col gap-3 reveal-element">
                ...
                <div class="lg:col-span-5 flex flex-col min-w-0 reveal-element delay-100">
            ...
            <div class="w-full h-px bg-gray-200 my-8 sm:my-12 reveal-element"></div>
...
function renderFooter() {
    ...
    DOM.footer.innerHTML = `
        <div class="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-12">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 border-b border-gray-800 pb-8 reveal-element">
            ...
            <p class="text-[10px] sm:text-xs text-gray-500 font-semibold tracking-wide text-center break-words reveal-element">${t().footerText}</p>
        </div>
    `;
}
